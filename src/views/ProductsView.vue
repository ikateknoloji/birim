<template>
  ...
  <div class="container mx-auto px-5 -mt-20 lg:-mt-40 bg-[#BB2525] mb-10 z-30 py-12">
    <div class="container mx-auto px-20 ">
      <div class="flex items-center justify-center mb-5  space-x-4">
        <input placeholder="OEM Kodu  veya Ürün Kodunu giriniz..."
          class="px-4 py-2 border block w-full focus:ring-2 focus:ring-blue-500 focus:outline-none" v-model="searchQuery"
          @keyup.enter="searchQueryFunction" />
        <button @click="searchQueryFunction" class="px-4 py-2 bg-white w-44 items-center"> Arama Yap</button>
      </div>
      ...
      <div class="mb-5 grid  grid-cols-1 md:grid-cols-2 gap-3">
        <div class="text-sm text-gray-900 font-montserrat font-medium border">
          ...
          <select v-model="selectedCategory" id="hs-hidden-select" class="focus:outline-none py-2 px-4 pr-9 block w-full">
        ...
        <div class="text-sm text-gray-900 font-montserrat font-medium border">
          ...
          <select v-model="selectedVehicleType" id="hs-hidden-select"
            class="focus:outline-none py-2 px-4 pr-9 block w-full">
      ...
      <div class="flex items-center justify-start">
        <button @click="resetValues"
          class="w-1/2 px-8 py-2 bg-gray-800 text-white hover:bg-slate-950 duration-300 delay-0 ease-linear rounded">Sıfırla</button>
      </div>
...
function scrollToTop() {
  myDiv.value.scrollIntoView({ behavior: 'smooth' });
}
